...
const emptyTracks: ProjectTrack[] = [];
const emptyTimelineTracks: ProjectTrack[] = [
  { id: "video-empty", kind: "Video", name: "Video", enabled: true, locked: false, clips: [] },
  { id: "overlay-empty", kind: "Overlay", name: "Overlay", enabled: true, locked: false, clips: [] },
  { id: "audio-empty", kind: "Audio", name: "Audio", enabled: true, locked: false, clips: [] },
  { id: "text-empty", kind: "Text", name: "Text", enabled: true, locked: false, clips: [] },
];
...
export function App() {
  ...
  const snapshotRef = useRef(emptyEditorSnapshot);
  ...
  const publishSnapshot = (next: EditorSnapshot) => {
  ...
  const tracks = snapshot.project?.sequence.tracks ?? emptyTracks;
  const allClips = useMemo(() => tracks.flatMap((track) => track.clips), [tracks]);
  const usedAssetIds = useMemo(() => new Set(allClips.map((clip) => clip.asset_id)), [allClips]);
  const visibleAssets = useMemo(
    () => snapshot.project?.assets.filter((asset) => projection.mediaFilter === "all" || usedAssetIds.has(asset.id)) ?? [],
    [projection.mediaFilter, snapshot.project, usedAssetIds],
  );
  const selectedClip = useMemo(() => allClips.find((clip) => clip.id === projection.selectedClipId) ?? null, [allClips, projection.selectedClipId]);
...
function Timeline({ snapshot, tracks, dispatch }: { snapshot: EditorSnapshot; tracks: ProjectTrack[]; dispatch: (command: EditorCommand) => Promise<EditorCommandResult> }) {
  ...
  const visibleTracks = tracks.length ? tracks : emptyTimelineTracks;
  const allClips = useMemo(() => tracks.flatMap((track) => track.clips), [tracks]);
  const clipDuration = useMemo(() => allClips.reduce((end, clip) => Math.max(end, clipEnd(clip)), 0), [allClips]);
  ...
  const selectedClip = allClips.find((clip) => clip.id === projection.selectedClipId) ?? null;
