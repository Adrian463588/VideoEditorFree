...
import { editorFacade, exportProfileOptions, type EditorCommand, type EditorCommandResult } from "./api/editorFacade";
import { Icon, type IconName } from "./components/Icon";
import { uiProjection, useUiProjection } from "./stores/uiProjection";
import { emptyEditorSnapshot, type EditorSnapshot, type ExportProfile, type ProjectAsset, type ProjectClip, type ProjectTrack, type Rational } from "./types/editor";
...
const mediaAssetKinds = new Set(["Video", "Audio", "Image"]);
...
const trackKindForAsset = (asset: ProjectAsset) => asset.kind === "Audio" ? "Audio" as const : asset.kind === "Subtitle" ? "Subtitle" as const : "Video" as const;
...
export function App() {
  ...
  const [bundleProfile, setBundleProfile] = useState<"core" | "subtitles">("core");
  ...
  const downloadBundle = async () => {
    setBundleBusy(true);
    try {
      await dispatch({ type: "bundleDownload", profile: bundleProfile });
    } finally {
      setBundleBusy(false);
    }
  };
  const addSelectedAsset = async () => {
    ...
    const kind = trackKindForAsset(selectedAsset);
    let document = snapshot.project;
    let track = document.sequence.tracks.find((candidate) => candidate.kind === kind && !candidate.locked);
    if (!track) {
      const nextTrack: ProjectTrack = {
        id: createId(`${kind.toLowerCase()}-layer`),
        kind,
        name: `${kind} layer ${document.sequence.tracks.filter((candidate) => candidate.kind === kind).length + 1}`,
        enabled: true,
        locked: false,
        clips: [],
      };
      const trackResult = await dispatch({
        type: "timelineApply",
        baseRevision: document.revision,
        operation: { AddTrack: { track: nextTrack } },
      });
    ...
    if (!track) {
      uiProjection.patch({ announcement: "BLOCKED — no editable timeline layer is available." });
      return;
    }
    const duration = selectedAsset.probe.duration_ticks;
    const timelineStart = track.clips.reduce((end, clip) => Math.max(end, clip.timeline_start + clip.timeline_duration), 0);
...
function EditorControls({ snapshot, selectedAsset, tracks, dispatch, onAddAsset }: { snapshot: EditorSnapshot; selectedAsset: ProjectAsset | null; tracks: ProjectTrack[]; dispatch: (command: EditorCommand) => Promise<EditorCommandResult>; onAddAsset: () => void }) {
  ...
  const targetId = audioTracks.some((track) => track.id === duckTargetId && track.id !== sourceId)
    ? duckTargetId
    : audioTracks.find((track) => track.id !== sourceId)?.id ?? "";
  ...
  const canGenerate = Boolean(project && selectedAsset && (selectedAsset.kind === "Video" || selectedAsset.kind === "Audio") && subtitleCapability.state === "READY" && !subtitleBusy);
  const generateSubtitles = async () => {
    if (!project || !selectedAsset || !canGenerate) return;
    setSubtitleBusy(true);
    try {
      await dispatch({ type: "subtitleGenerate", assetId: selectedAsset.id, language: subtitleLanguage, baseRevision: project.revision, trackId: subtitleTrackId });
    } finally {
      setSubtitleBusy(false);
    }
  };
  const toggleDucking = () => {
    if (!project || !targetTrack || !sourceId || sourceId === targetId || targetTrack.locked) return;
    void dispatch({
      type: "timelineApply",
      ...
      operation: {
        SetTrackDucking: {
          track_id: targetId,
          ducking: targetTrack.ducking ? null : { source_track_id: sourceId, threshold_db: -24, ratio: 6, attack_ms: 20, release_ms: 300 },
        },
      },
    });
  };
  return <section className="panel controls-panel"><SectionHeading icon="layout" eyebrow="EDITING" title="Layers & automation" /><div className="control-group"><div className="control-heading"><strong>Selected layer</strong><span>{selectedAsset ? assetName(selectedAsset) : "No asset"}</span></div><button className="button button-primary button-small" disabled={!selectedAsset || selectedAsset.status !== "Available" || !selectedAsset.probe || selectedAsset.probe.duration_ticks <= 0} onClick={onAddAsset}><Icon name="plus" size={14} /> Add to timeline</button><small className="control-help">Adds the real selected asset to an editable video or audio layer.</small></div><div className="control-group"><div className="control-heading"><strong>Auto subtitles</strong><StatusPill state={subtitleCapability.state}>{subtitleCapability.state}</StatusPill></div><label className="control-label" htmlFor="subtitle-language">Spoken language</label><select id="subtitle-language" className="control-select" value={subtitleLanguage} onChange={(event) => setSubtitleLanguage(event.target.value)}>{subtitleLanguages.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><button className="button button-quiet button-small" disabled={!canGenerate} onClick={() => void generateSubtitles()}><Icon name="message" size={14} /> {subtitleBusy ? "Generating…" : "Generate subtitles"}</button><small className="control-help">Local Whisper captions are added to a subtitle layer. The selected language is the spoken language, not cloud translation.</small></div><div className="control-group"><div className="control-heading"><strong>Audio ducking</strong><StatusPill state={snapshot.capabilities.audioDucking.state}>{snapshot.capabilities.audioDucking.state}</StatusPill></div>{audioTracks.length < 2 ? <small className="control-help">Add two audio layers to enable sidechain ducking.</small> : <><label className="control-label" htmlFor="duck-source">Ducking source</label><select id="duck-source" className="control-select" value={sourceId} onChange={(event) => setDuckSourceId(event.target.value)}>{audioTracks.map((track) => <option key={track.id} value={track.id}>{track.name}</option>)}</select><label className="control-label" htmlFor="duck-target">Lower this layer</label><select id="duck-target" className="control-select" value={targetId} onChange={(event) => setDuckTargetId(event.target.value)}>{audioTracks.filter((track) => track.id !== sourceId).map((track) => <option key={track.id} value={track.id}>{track.name}</option>)}</select><button className="button button-quiet button-small" disabled={snapshot.capabilities.audioDucking.state !== "READY" || !targetTrack || targetTrack.locked || sourceId === targetId} onClick={toggleDucking}><Icon name="volume" size={14} /> {targetTrack?.ducking ? "Disable ducking" : "Enable ducking"}</button><small className="control-help">Sidechain compression uses the source layer to lower the target during speech or music.</small></>}</div></section>;
}
